...
from fantasy.settings import API_BASE, MIN_REQUEST_INTERVAL, USER_AGENT
from fantasy.sources.laliga.auth import TokenStore
...
class FantasyClient:
    ...
    def market_value(self, player_id: str) -> Any:
        """A player's daily price history. Public: no bearer required.

        The whole season arrives in one response, so history never has to be
        accumulated by polling — it can be re-fetched and back-filled at will.
        """
        return self.get(f"{self.base}/player/{player_id}/market-value")
