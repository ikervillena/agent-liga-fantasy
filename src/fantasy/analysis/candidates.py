        ...
        for owned in rival.squad:
            ...
            intents.append(
                Intent(
                    ...
                    # Deliberately the lock instant and not `opens_at`: when a
                    # clause is already open the latter is just "now", and an
                    # identity built on it changes with every poll.
                    anchor=owned.clause_locked_until,
                    player_id=owned.id,
                    player_name=owned.name,
                    amount=price,
                    expected_gain=gain,
