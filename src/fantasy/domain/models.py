...
class Team(BaseModel):
    ...
    #: The game's own verdict on whether this squad will score. It goes false
    #: while the balance is negative, and a squad that is still false when the
    #: matchday opens scores nothing — so it is read, never inferred.
...
class Matchday(BaseModel):
    """A matchday and, crucially, the instant it stops accepting changes.

    `opens_at` is the first kick-off. It is the only deadline that matters: the
    eleven is frozen then, and a squad sitting in the red at that moment scores
    nothing at all for the week. Everything that happens afterwards is noise.
    """
...
class LeagueConfig(BaseModel):
    """What this particular league allows, read from the API rather than assumed.

    Two families of setting change what advice even makes sense. The premium
    features decide whether a captain or a bench exists at all — in a league
    with them switched off, mentioning either is noise. And
    `clause_block_hours` is the window before a matchday in which clauses
    cannot be paid: the default is none, but a league can set 24 to 72 hours,
    which moves the last useful moment to sign somebody by a whole day.
    """

    model_config = ConfigDict(frozen=True)

    buyout_clauses: bool = True
    captain: bool = False
    bench: bool = False
    free_formations: bool = False
    coach: bool = False
    loans: bool = False
    #: Hours before the first kick-off during which clauses cannot be paid.
    clause_block_hours: int = 0
...
class ValuePoint(BaseModel):
    """One day in a player's price history.

    `bids` is how many managers had an offer in for him that day. It is the
    only demand signal the game exposes, and it leads price: interest shows up
    there before it shows up in the valuation.
    """
...
class ValueSeries(BaseModel):
    """A player's daily market value, oldest first.

    The game recomputes every valuation overnight, so this is the series that
    says who is appreciating and who is bleeding. Buying a player three days
    before a run of rises is how a squad outgrows the league without winning a
    single extra point, which makes this the most profitable data in the game
    and the reason it is fetched rather than inferred.
    """
    ...
    @property
    def latest(self) -> ValuePoint | None:
        return self.points[-1] if self.points else None

    def on_or_before(self, day: date) -> ValuePoint | None:
        """The last observation up to `day`, which is not always `day` itself.

        Gaps happen — a missed overnight job, a player joining mid-season — so
        callers asking "what was he worth a week ago" must not assume the day
        they asked for exists.
        """
        candidates = [p for p in self.points if p.date <= day]
