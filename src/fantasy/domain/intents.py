...
class Intent(BaseModel):
    ...
    #: The deterministic instant this opportunity hangs off — the moment a
    #: clause opens, or a matchday deadline. It is what gives the intent a
    #: stable identity across runs, which `execute_at` cannot: for an
    #: opportunity that is already live, `execute_at` is simply "now" and moves
    #: with every poll.
    ...
    @property
    def key(self) -> str:
        """Stable identity: which operation, on which target, in which window.

        What is *deliberately* absent is as important as what is present.

        The price is not part of it. Market values move every night, so pricing
        the identity meant "pay Mandi's clause" became a brand-new decision
        each morning and was put to the manager again — fifteen times over, in
        practice. Whether the price still makes the deal acceptable is a
        question about conditions, not identity, and `Conditions` already
        answers it with an explicit tolerance.

        `execute_at` is not part of it either. For an opportunity that is
        already live it collapses to "now", which changes on every poll and
        produced the same spam on a fifteen-minute cycle. The window `anchor`
        is used instead, precisely because it does not move.

        The rationale and expected gain are excluded too: they are commentary,
        and rewording an explanation must not resurrect a settled decision.
        """
        raw = "|".join(
            [
                self.kind.value,
                ...
                self.market_id or "",
                self.offer_id or "",
                self.anchor.isoformat(timespec="minutes") if self.anchor else "",
            ]
        )
