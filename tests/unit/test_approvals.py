...
from fantasy.domain.models import PlayerStatus, SquadRole
...
class TestIdentity:
    ...
    def test_a_different_price_is_the_same_operation(self) -> None:
        """Price belongs to the conditions, not to the identity.

        This assertion used to read the other way round, and that is exactly
        what made the agent unusable: market values move every night, so
        pricing the identity turned one decision into a new one every morning
        and asked the manager again each time. Whether the new price is still
        acceptable is answered by revalidation below, which is the mechanism
        built for it.
        """
